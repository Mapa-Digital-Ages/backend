...
class TestResourceService(unittest.TestCase):
    ...
    def test_upload_resource_validates_magic_bytes_pdf(self):
        ...
        async def run_test():
            session, engine = await _create_session()
            async with session as db_session:
                subject = Subject(name="Security Test", slug="security-test")
                db_session.add(subject)
                await db_session.commit()
                content = Content(subject_id=subject.id, name="Security")
                db_session.add(content)
                await db_session.commit()
                ...
                # Arquivo malicioso: extensão .pdf mas conteúdo de .exe
                malicious_content = b"MZ\x90\x00"  # Magic bytes de .exe
                result = await service.upload_resource(
                    session=db_session,
    ...
    def test_upload_resource_validates_magic_bytes_video(self):
        ...
        async def run_test():
            session, engine = await _create_session()
            async with session as db_session:
                subject = Subject(name="Video Test", slug="video-test")
                db_session.add(subject)
                await db_session.commit()
                content = Content(subject_id=subject.id, name="Video Content")
                db_session.add(content)
                await db_session.commit()
                ...
                # Arquivo malicioso: extensão .mp4 mas conteúdo de texto
                malicious_content = b"This is not a video file"
                result = await service.upload_resource(
                    session=db_session,
                    file_bytes=malicious_content,
    ...
    def test_upload_resource_rejects_file_for_link_type(self):
        ...
        async def run_test():
            session, engine = await _create_session()
            async with session as db_session:
                subject = Subject(name="Link Test", slug="link-test")
                db_session.add(subject)
                await db_session.commit()
                content = Content(subject_id=subject.id, name="Link Content")
                db_session.add(content)
                await db_session.commit()
                ...
                # Tentar enviar arquivo com type='link' deve falhar
                result = await service.upload_resource(
                    session=db_session,
                ...
                self.assertEqual(result, "invalid_resource_type")
